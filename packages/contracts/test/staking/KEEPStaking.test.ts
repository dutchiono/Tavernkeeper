...
import { KEEPStaking, KeepToken, TheCellarV3 } from "../../typechain-types";
import { SignerWithAddress } from "@nomicfoundation/hardhat-ethers/signers";

describe("KEEPStaking", function () {
    let keepStaking: KEEPStaking;
    ...
    beforeEach(async function () {
        ...
        keepStaking = await KEEPStakingFactory.deploy(
        ...
        // Mint some KEEP tokens to users
        const amount = ethers.parseEther("1000");
        await keepToken.mint(user1.address, amount);
        await keepToken.mint(user2.address, amount);
    ...
    describe("Staking", function () {
        it("Should allow users to stake KEEP tokens", async function () {
            const stakeAmount = ethers.parseEther("100");
            await keepToken.connect(user1).approve(await keepStaking.getAddress(), stakeAmount);
            await keepStaking.connect(user1).stake(stakeAmount, 0);

            const userStake = await keepStaking.getUserStake(user1.address);
            expect(userStake.amount).to.equal(stakeAmount);
            expect(await keepToken.balanceOf(await keepStaking.getAddress())).to.equal(stakeAmount);
        });
        ...
        it("Should apply lock multiplier correctly", async function () {
            ...
            await keepStaking.connect(user1).stake(stakeAmount, 365);
        ...
        it("Should prevent unstaking before lock expires", async function () {
            const stakeAmount = ethers.parseEther("100");
            await keepToken.connect(user1).approve(await keepStaking.getAddress(), stakeAmount);
            await keepStaking.connect(user1).stake(stakeAmount, 30); // 30 day lock

            await expect(
                keepStaking.connect(user1).unstake(ethers.parseEther("50"))
        ...
        it("Should allow unstaking after lock expires", async function () {
            const stakeAmount = ethers.parseEther("100");
            await keepToken.connect(user1).approve(await keepStaking.getAddress(), stakeAmount);
            await keepStaking.connect(user1).stake(stakeAmount, 1); // 1 day lock

            // Fast forward time
            await ethers.provider.send("evm_increaseTime", [86401]); // 1 day + 1 second
            await ethers.provider.send("evm_mine", []);

            await keepStaking.connect(user1).unstake(ethers.parseEther("50"));
            const userStake = await keepStaking.getUserStake(user1.address);
    ...
    describe("Rewards", function () {
        it("Should distribute rewards proportionally", async function () {
            ...
            await keepToken.connect(user1).approve(await keepStaking.getAddress(), stakeAmount1);
            await keepStaking.connect(user1).stake(stakeAmount1, 0);

            // User2 stakes with no lock
            await keepToken.connect(user2).approve(await keepStaking.getAddress(), stakeAmount2);
            await keepStaking.connect(user2).stake(stakeAmount2, 0);

            // Deposit rewards
            const rewardAmount = ethers.parseEther("1000");
            await keepToken.approve(await keepStaking.getAddress(), rewardAmount);
            await keepStaking.depositRewards(rewardAmount);

            // User1 should get 1/3 of rewards (100 / 300)
            const pending1 = await keepStaking.getPendingRewards(user1.address);
            expect(pending1).to.be.closeTo(ethers.parseEther("333.33"), ethers.parseEther("0.1"));

            // User2 should get 2/3 of rewards (200 / 300)
            const pending2 = await keepStaking.getPendingRewards(user2.address);
        ...
        it("Should apply lock multiplier to rewards", async function () {
            ...
            const pending = await keepStaking.getPendingRewards(user1.address);
        ...
        it("Should allow users to claim rewards", async function () {
            ...
            await keepStaking.connect(user1).claimRewards();
    ...
    describe("Access Control", function () {
        it("Should only allow owner or cellar to deposit rewards", async function () {
            const rewardAmount = ethers.parseEther("1000");
            await keepToken.connect(user1).approve(await keepStaking.getAddress(), rewardAmount);

            await expect(
                keepStaking.connect(user1).depositRewards(rewardAmount)
