...
import { HeroClass, HeroColors, Gender } from '../types';
        ...
        for (let x = minX; x < maxX; x++) {
            ...
            if (charSet.has(char)) {
                ...
                if (ny >= 0 && ny < size && nx >= 0 && nx < size) {
                     // Only overwrite if not replacing with transparent (unless desired, but here we stack)
                     // Actually, we should just move it.
                     // Simple approach: Re-write to newMap
                     newMap[ny][nx] = char;
                }
            }
        }
