...
export const ForgePanel: React.FC<ForgePanelProps> = ({ 
  ...
  const baseStyles = "relative border-4 p-6 shadow-xl transition-all duration-300";
  
  const variants = {
    wood: "bg-[#2a1d17] border-[#5c3a1e] text-[#fcdfa6]", // Dark wood theme from request
    paper: "bg-[#eaddcf] border-[#8c7b63] text-[#3e3224]", // Paper theme
  };
...
export const ForgeButton: React.FC<ForgeButtonProps> = ({ 
  ...
  const base = "px-6 py-3 font-bold text-xs uppercase tracking-widest transition-all transform active:scale-[0.98] border-b-4 border-r-4 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed";
